...
function PrimusCluster(
  primus,
  { cluster: { channel = "primus", ttl = 86400, redis = {} } = {} } = {}
) {
  this.primus = primus;
...
PrimusCluster.prototype.dispatchMessage = function dispatchMessage(msg) {
  this.primus.decoder(
    ...
    function (err, msg) {
      ...
      if (err)
        return (
          this.primus.listeners("error").length &&
          this.primus.emit("error", err)
        );
...
PrimusCluster.prototype.publish = function publish(data, type, opts = {}) {
  ...
  this.primus.encoder(message, (err, msg) => {
    ...
    if (err) {
      return (
        this.primus.listeners("error").length && this.primus.emit("error", err)
      );
    }
