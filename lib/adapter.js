...
Adapter.prototype.clients = function clients(room, cb) {
  this.client.smembers(roomKey.format(room), function(err, clients) {
    cb(err, clients);
    this.clean(clients);
  }.bind(this));
};
...
/**
 * clean expired clients from the all rooms
 * @param  {[type]} clients [description]
 * @return {[type]}         [description]
 */
Adapter.prototype.clean = function(clients) {
  var multi = this.client.multi();

  clients.forEach(function(c) {
    multi.smembers(socketKey.format(c));
  });
